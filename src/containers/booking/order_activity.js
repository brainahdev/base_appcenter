...
class Activity extends Component {
	...
	if(this.callInvoked == 0){
		...
		this.props.item_user_messages(payload).then((resp) => {
			...
			if(p==1)
				...
				datares=this.state.dataList.concat(messagee);
			
			var cpage = page + 1;
			this.callInvoked=0;
			if(this.state.page == 1 && messagee.length ==0)
				this.setState({nodata:1,loading:false,page:cpage,dataList:datares,dataSource: this.state.dataSource.cloneWithRows(datares)});
			else
				this.setState({page:cpage,loading:false,dataList:datares,dataSource: this.state.dataSource.cloneWithRows(datares)});
		}).catch(() => {
			console.log("error");
		});
	}
  ...
  _renderRow=(data)=>{
   ...
   checkout=(id)=>{
	   if(this.state.claim_amount.trim() == '' || !this.validateAmount(this.state.claim_amount) || this.state.claim_amount>100){
		  Alert.alert(
			  AppConfig.appName,
		  ...
		  Alert.alert(
			  AppConfig.appName,
			  Strings.props[this.state.userLang].areyousurewanttocheckout,
			  [
				{text: 'OK', onPress: () => {
					/////////////////////
						var payload = {"type":"checkout","id":id,"call_from":"activity_status_update","claim_request_amount":this.state.claim_amount};
						this.props.bookings(payload).then((resp) => {
							...
							if(resp.Success){
								...
								this.setState({is_checkout:false});
								this.activity_data();
							}
							else if(resp.message){
								Alert.alert(
								  AppConfig.appName,
								  Strings.props[this.state.userLang].unabletoprocess,
								  [
								  {text: 'OK', onPress: () => console.log('OK Pressed')},
								  ],
								  { cancelable: false }
   ...
   update_status=(data)=>{
	   ...
	   var type = data.type;
	   if(type=='checkout'){
		   //http://bookorrent.servicepg.develag.com/public/api/vehicle_rentals/76/checkout
		   //claim_request_amount	10
		   //{"Success":"VehicleRental has been checked-out successfully"}
		   this.setState({is_checkout:true});
	   }else{
   ...
   feedback=()=>{
	   if(this.state.rating_value == 0){
		   ...
		   //feedback	Good 
			//item_user_id	41
			//rating	3
			var payload = {"type":"post_host_review","rating":this.state.rating_value,"item_user_id":this.state.data.id,"feedback":this.state.private_note_value};
			console.log("payload" + JSON.stringify(payload));
			this.setState({loading:true});
			this.props.host(payload).then((resp) => {
				if(resp.Success){
					Alert.alert(
					  AppConfig.appName,
					  resp.Success,
  ...
  render = () => 
	{
		...
		if(view_data.item_user_status)
			console.log("view_data.item_user_status.id" + view_data.item_user_status.id);
    ...
    <View style={[AppStyles.container, styles.background],{justifyContent:'center',alignItems:'center'}}> 
		<View style={{width: AppSizes.screen.width-10,flexDirection:'row',margin:10,backgroundColor:AppColors.brand.primary,padding:10,borderRadius:15}}>
			<ScrollView showsVerticalScrollIndicator={false}>
					...
					<View>
						...
						<View style={{flexDirection:'row'}}>
							{(view_data.item_user_status && view_data.item_user_status.id == 2) ? 
								<View style={{flexDirection:'row'}}> 
									<TouchableOpacity style={styles.selected_tab} onPress={this.update_status.bind(this,{"id":view_data.id,"item_user_status_id":view_data.item_user_status.id,"type":"confirm"})}> 
										<Text style={[styles.headerGrey,styles.whiteText,AppStyles.regularFontText]}>{Strings.props[this.state.userLang].accept}</Text>
									</TouchableOpacity>
									<TouchableOpacity style={styles.selected_tab} onPress={this.update_status.bind(this,{"id":view_data.id,"item_user_status_id":view_data.item_user_status.id,"type":"reject"})}> 
										<Text style={[styles.headerGrey,styles.whiteText,AppStyles.regularFontText]}>{Strings.props[this.state.userLang].reject}</Text>
									</TouchableOpacity>
								</View>
							: null}
							{(view_data.item_user_status && view_data.item_user_status.id == 7) ? 
								<View>
									<TouchableOpacity style={styles.selected_tab} onPress={this.update_status.bind(this,{"id":view_data.id,"item_user_status_id":view_data.item_user_status.id,"type":"checkin"})}> 
										<Text style={[styles.headerGrey,styles.whiteText,AppStyles.regularFontText]}>{Strings.props[this.state.userLang].checkin}</Text>
									</TouchableOpacity>
								</View>
							: null}
							{(view_data.item_user_status && view_data.item_user_status.id == 12) ? 
								<View>
									<TouchableOpacity style={styles.selected_tab} onPress={this.update_status.bind(this,{"id":view_data.id,"item_user_status_id":view_data.item_user_status.id,"type":"checkout"})}> 
										<Text style={[styles.headerGrey,styles.whiteText,AppStyles.regularFontText]}>{Strings.props[this.state.userLang].checkout}</Text>
									</TouchableOpacity>
								</View>
							: null}
							{(view_data.item_user_status && ((view_data.item_user_status.id == 8 && view_data.is_dispute == 0) || view_data.item_user_status.id == 9)) ? 
								<View>
				...
				<Spacer size={25}/>
				{/*tabs*/}
				{(view_data.item_user_status && (view_data.item_user_status.id == 8 || view_data.item_user_status.id == 9)) ? 
					<View>
				...
				{(view_data.item_user_status && (view_data.item_user_status.id == 8  || view_data.item_user_status.id == 9) && this.state.selected_index == 2) ? 
					<View>
		...
		{(this.state.is_checkout) ? 
			<View style={{justifyContent:'center',alignItems:'center',width:AppSizes.screen.width,height:AppSizes.screen.height,position:'absolute',backgroundColor:'rgba(0,0,0,0.8)'}}>
				<View style={{justifyContent:'flex-start',alignItems:'flex-start',width:AppSizes.screen.width-40,height:AppSizes.screen.height/3,backgroundColor:AppColors.brand.white,borderRadius:10,padding:15}}>
					<View><Text>Check Out Details:</Text></View>
					<Spacer size={20}/>
					<LblFormInput lblTxt={'Claim Request Amount'} placeholderTxt={'Claim Request Amount'} value={this.state.claim_amount} width={AppSizes.screen.width-70} onChangeText={(text) => this.setState({claim_amount:text})}/>
					<View style={{width:AppSizes.screen.width-60,justifyContent:'center',alignItems:'center'}}>
						<Button 
							small
							title={Strings.props[this.state.userLang].checkout} 
							backgroundColor={'#33BB76'}
							onPress={this.checkout.bind(this,view_data.id)}
							borderRadius = {50}
							fontSize={15}
							buttonStyle={{padding:14,paddingLeft:20,paddingRight:20}}
							outlined
						  />
					</View>
				</View>
