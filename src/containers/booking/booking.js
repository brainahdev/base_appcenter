...
class Booking extends Component {
  ...
  onEndReached=()=>{
	this.get_bookings(this.state.cfilter,0);
  }
  _renderRow=(data)=>{
	  console.log("data===" + JSON.stringify(data)); 
     return (
		<TouchableOpacity style={{width: AppSizes.screen.width-10,margin:5,borderBottomWidth:1,padding:10,borderRadius:15,backgroundColor:AppColors.brand.primary}} onPress={()=>{Actions.Activity({'data':JSON.stringify(data)})}}> 
			<View style={{flexDirection:'row',justifyContent:'space-between'}}>
				<View><Text >Booking ID: #{data.id}</Text></View>
				<View style={{backgroundColor:AppColors.brand.btnColor,height:18,borderRadius:20,justifyContent:'center',alignItems:'center',padding:5 }}><Text style={{fontSize:8,color:AppColors.brand.white,lineHeight:10}}>{data.item_user_status.name}</Text></View>
			</View>
			<Spacer size={5}/>
			<View style={{flexDirection:'row',justifyContent:'space-between'}}>
				<View><Text style={{fontSize:16}}>{data.item_userable.name}</Text></View>
				<View><Text >{Strings.props[this.state.userLang].host}: {data.item_userable.user.username}</Text></View> 
			</View>
			<Spacer size={5}/>
			<View style={{flexDirection:'row',justifyContent:'space-between'}}>
				<View><Text style={{fontSize:10}}>{Strings.props[this.state.userLang].from}: {data.item_booking_start_date}</Text></View>
				<View><Text style={{fontSize:10}}>{Strings.props[this.state.userLang].to}: {data.item_booking_end_date}</Text></View>
			</View>
			<Spacer size={5}/>
			<View style={{flexDirection:'row',justifyContent:'space-between'}}>
				<View><Text style={{fontSize:10}}>{Strings.props[this.state.userLang].bookedon}: {data.created_at}</Text></View>
				<View><Text style={{fontSize:10}}>{Strings.props[this.state.userLang].Gross}: ${data.total_amount}</Text></View>
			</View> 
		</TouchableOpacity>  
  ...
  filter=(id)=>{
	  ...
	  this.setState({cfilter:id,page:1,dataList:[],nodata:0},this.get_bookings(cf,1)); 
  }
  render = () => 
    {
		...
		var fpendingt = (this.state.cfilter == 'pending')? [styles.headerGrey,styles.whiteText,AppStyles.regularFontText] :[styles.headerGrey,AppStyles.regularFontText];
    ...
    <View style={[AppStyles.container, styles.background]}>
		...
		<ScrollView showsHorizontalScrollIndicator={false} horizontal={true} style={{paddingTop:5,backgroundColor:'#303030',width:AppSizes.screen.width}} >
			...
			<TouchableOpacity disabled={disabled} onPress={this.filter.bind(this,'pending')} style={fpending}>
				<Text style={fpendingt}>{Strings.props[this.state.userLang].paymentpending}</Text>
			</TouchableOpacity>
