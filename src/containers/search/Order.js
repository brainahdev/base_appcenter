...
const mapkey = "AIzaSyCOWGW5CCNcl5pC-iITlnTrPcw-Ur4BPXg";
...
class Order extends Component {
  ...
  apply_coupon=()=>{
	  if(this.state.couponcode == ''){
		  Alert.alert(
			  AppConfig.appName,
			  Strings.props[this.state.userLang].entercouponcode,
			  [
  ...
  update=()=>{
	 ...
	 if(this.state.firstname == '')
	  ...
	  else if(this.state.mobile == '' || !this.validateMobile(this.state.mobile) )
	  {
		  Alert.alert(
			  AppConfig.appName,
			  Strings.props[this.state.userLang].entervalidmobilenumber,
			  [
			  {text: 'OK', onPress: () => console.log('OK Pressed')},
			  ],
			  { cancelable: false }
	  ...
	  else if(this.state.address == '')
	  {
		  Alert.alert(
			  AppConfig.appName,
			  Strings.props[this.state.userLang].addresspickmessage,
			  [
			  {text: 'OK', onPress: () => console.log('OK Pressed')},
			  ],
			  { cancelable: false }
  ...
  render = () =>
    {
		...
		if(this.state.data && this.state.data.item_userable && this.state.data.item_userable.vehicle_type && this.state.data.item_userable.vehicle_type.vehicle_type_extra_accessory){
			...
			for (var j = 0; j < extra_accessory.length; j++)
			{
				...
				if(xdata){
					accessories_data.push(<View key={j}><View><Text>{xdata.name}</Text></View><View><Text style={{fontSize:9}}>{xdata.description}</Text></View><View style={{marginTop:5}}><Text>{xrate.rate} / {Strings.props[this.state.userLang].perrental}</Text></View></View> 
					)
				}
			 }
		}
		...
		if(this.state.data && this.state.data.item_userable && this.state.data.item_userable.vehicle_type && this.state.data.item_userable.vehicle_type.vehicle_type_insurance){
			...
			for (var j = 0; j < extra_accessory.length; j++)
			{
				...
				if(xdata){
					insurance_data.push(<View key={j}><View><Text >{xdata.name}</Text></View><View><Text style={{fontSize:9}}>{xdata.description}</Text></View><View style={{marginTop:5}}><Text >{xrate.rate} / {Strings.props[this.state.userLang].perrental}</Text></View></View> 
					)
				}
			 }
		}
		...
		if(this.state.data && this.state.data.item_userable && this.state.data.item_userable.vehicle_type && this.state.data.item_userable.vehicle_type.vehicle_type_fuel_option){
			...
			for (var j = 0; j < extra_accessory.length; j++)
			{
				...
				var xrate = extra_accessory[j]; 
				if(xdata){
					fuel_data.push(<View key={j}><View><Text>{xdata.name}</Text></View><View><Text style={{fontSize:9}}>{xdata.description}</Text></View><View style={{marginTop:5}}><Text>{xrate.rate} / {Strings.props[this.state.userLang].perrental}</Text></View></View> 
					)
				}
			 }
		}
		...
		if(this.state.data && this.state.data.date_diff && this.state.data.date_diff.total_days && !this.state.data.date_diff.total_hours){
			days_hours = this.state.data.date_diff.total_days +" "+Strings.props[this.state.userLang].days;
			...
			days_hours = this.state.data.date_diff.total_hours +" "+Strings.props[this.state.userLang].hours;
			days_hours_amount = this.state.data.date_diff.total_hours * this.state.data.item_userable.per_hour_amount;
		}
		else if(this.state.data && this.state.data.date_diff && this.state.data.date_diff.total_days && this.state.data.date_diff.total_hours){
			days_hours = this.state.data.date_diff.total_days +" "+Strings.props[this.state.userLang].days +" "+this.state.data.date_diff.total_hours +" "+Strings.props[this.state.userLang].hours;
		...
		<ScrollView>
			<View style={{flexDirection:'column'}}>
				...
				<View style={{margin:20,backgroundColor:AppColors.brand.white}}>
					...
					<Spacer size={5}/>
					<Text style={styles.headerGrey}>${(this.state.data)?this.state.data.item_userable.per_day_amount:0} / {Strings.props[this.state.userLang].day}</Text>
					<Spacer size={5}/>
					<Text style={styles.headerGrey}>${(this.state.data)?this.state.data.item_userable.per_hour_amount:0} / {Strings.props[this.state.userLang].hour}</Text>
				</View>
			</View>
			<View style={{margin:20,marginTop:0,backgroundColor:AppColors.brand.white,justifyContent:'center',alignItems:'flex-start'}}>
				<View><Text style={styles.header}>{Strings.props[this.state.userLang].travellerdetails}</Text></View>
				<Spacer size={5}/>
				<View style={{width:AppSizes.screen.width-50,backgroundColor:'transparent'}}>
					<LblFormInput placeholderTxt={Strings.props[this.state.userLang].firstname} lblTxt={Strings.props[this.state.userLang].firstname} value={this.state.firstname} onChangeText={(firstname) => this.setState({firstname})}/>
				</View>
				<View style={{width:AppSizes.screen.width-50,backgroundColor:'transparent'}}>
					<LblFormInput placeholderTxt={Strings.props[this.state.userLang].lastname} lblTxt={Strings.props[this.state.userLang].lastname} value={this.state.lastname} onChangeText={(lastname) => this.setState({lastname})}/>
				</View>
				<View style={{width:AppSizes.screen.width-50,backgroundColor:'transparent'}}>
					<LblFormInput placeholderTxt={Strings.props[this.state.userLang].email} lblTxt={Strings.props[this.state.userLang].email} value={this.state.email} onChangeText={(email) => this.setState({email})}/>
				</View>
				<View style={{width:AppSizes.screen.width-50,backgroundColor:'transparent'}}>
					<LblFormInput placeholderTxt={Strings.props[this.state.userLang].mobile} lblTxt={Strings.props[this.state.userLang].mobile} value={this.state.mobile} onChangeText={(mobile) => this.setState({mobile})}/>
				</View>
				<View style={{width:AppSizes.screen.width-50,backgroundColor:'transparent',margin:0,padding:0}}>
					<View style={{width:AppSizes.screen.width-50,margin:0,padding:0}}> 
						<Text style={{color: AppColors.brand.black, fontSize: 14,},[AppStyles.regularFontText]}>{Strings.props[this.state.userLang].address}:</Text>
					</View>
				</View> 
				...
				<GooglePlacesAutocomplete
					  placeholder={(this.state.address)?this.state.address:Strings.props[this.state.userLang].address}
					  ...
					  styles={{
						...
						description: {
						  fontFamily:'OpenSans',fontSize:12,color: AppColors.brand.txtinputcolor
						},
				...
				{accessories_data ? 
					<View >
						<View style={{justifyContent:'center',alignItems:'flex-start',marginBottom:10}}>
							<CheckBox
							  left
							  title={Strings.props[this.state.userLang].accessories}
				...
				{insurance_data ? 
					<View >
						<View style={{justifyContent:'center',alignItems:'flex-start',marginBottom:10}}>
							<CheckBox
							  left
							  title={Strings.props[this.state.userLang].insurance}
				...
				{fuel_data ? 
					<View >
						<View style={{justifyContent:'center',alignItems:'flex-start',marginBottom:10}}>
							<CheckBox
							  left
							  title={Strings.props[this.state.userLang].fueloptions}
				...
				: null}
				<Spacer size={20} />
				<View ><Text style={styles.header}>{Strings.props[this.state.userLang].coupons}:</Text></View>
				<Spacer size={10} /> 
				<View style={{flexDirection:'row'}}>
					<View style={{width:150}}>
						<LblFormInput width={150} placeholderTxt={Strings.props[this.state.userLang].couponcode} lblTxt={Strings.props[this.state.userLang].couponcode} value={this.state.couponcode} onChangeText={(couponcode) => this.setState({couponcode})}/>
					</View>
					<TouchableOpacity onPress={this.apply_coupon} style={{justifyContent:'center',alignItems:'center',backgroundColor:'#33BB76',width:100,padding:10,height:30,borderRadius:50,paddingBottom:10}} >
						<Text style={{color:AppColors.brand.white}}>{Strings.props[this.state.userLang].apply}</Text>
					</TouchableOpacity>
				</View>
				<Spacer size={20} />
				<View ><Text style={styles.header}>{Strings.props[this.state.userLang].faredetails}</Text></View>
				<Spacer size={10} /> 
				<View style={{justifyContent:'center',alignItems:'center'}}>
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text style={[AppStyles.boldedFontText]}>{Strings.props[this.state.userLang].details}</Text></View>
						<View style={styles.fare_day}><Text style={[AppStyles.boldedFontText]}>{Strings.props[this.state.userLang].days}</Text></View>
						<View style={styles.fare_day}><Text style={[AppStyles.boldedFontText]}>{Strings.props[this.state.userLang].total}</Text></View>
					</View>
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].tax}</Text></View>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].deposit}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
						<View style={styles.fare_day}><Text>(+) ${(this.state.data)?this.state.data.deposit_amount:0}</Text></View>
					</View>
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].onewaydropoffcharge}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
						<View style={styles.fare_day}><Text>(+) ${(this.state.data)?this.state.data.drop_location_differ_charges:0}</Text></View>
					</View>
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].droplocationdifferadditionalfee}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
						<View style={styles.fare_day}><Text>(+) ${(this.state.data)?this.state.data.drop_location_differ_additional_fee:0}</Text></View>
					</View>
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].surchargeamount}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].extraaccessoryamount}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
						<View style={styles.fare_day}><Text>(+) ${(this.state.data)?this.state.data.extra_accessory_amount:0}</Text></View>
					</View>
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].fuelamount}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].insuranceamount}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].typediscountamount}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].discount}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
					...
					<Spacer size={10} />
					<View style={{flexDirection:'row'}}>
						<View style={styles.fare_title}><Text>{Strings.props[this.state.userLang].totalprice}</Text></View>
						<View style={styles.fare_day}><Text></Text></View>
					...
					<View style={{width:AppSizes.screen.width-40,flexDirection:'row'}}>
						<Button
								title={Strings.props[this.state.userLang].update}
