...
class Search extends Component {
  ...
  constructor(props) {
	...
	this.price_type = 'day';
	this.max_day_price = parseFloat(this.props.vehicle_filters.vehicle_type_price.max_day_price);
	this.min_day_price = parseFloat(this.props.vehicle_filters.vehicle_type_price.min_day_price);
	this.price=[this.min_day_price, this.max_day_price];
	if(this.props.payload && this.props.payload.price_type == 'day'){
		this.price=[parseFloat(this.props.payload.min_price), parseFloat(this.props.payload.max_price)];
	}
	
	this.max_hour_price = parseFloat(this.props.vehicle_filters.vehicle_type_price.max_hour_price);
	this.min_hour_price = parseFloat(this.props.vehicle_filters.vehicle_type_price.min_hour_price)
	...
	if(this.props.payload && this.props.payload.price_type == 'hour'){
		...
		this.hourprice=[parseFloat(this.props.payload.min_price), parseFloat(this.props.payload.max_price)];
	}
	...
	if(this.props.payload && this.props.payload.seat_min){
		this.seat=[parseFloat(this.props.payload.seat_min), parseFloat(this.props.payload.seat_max)];
	} 
	
	var pref = [];
	if(this.props.payload && this.props.payload.ac)
		pref.push('ac');
	if(this.props.payload && this.props.payload.non_ac)
		pref.push('non_ac');
	if(this.props.payload && this.props.payload.airbag)
		pref.push('airbag');
    ...
    this.state = {
	  ...
	  userLang:'en',
	  selected_car_types:(this.props.payload && this.props.payload.vehicle_type)?this.props.payload.vehicle_type:[],
	  selected_fuel_types:(this.props.payload && this.props.payload.fuel_type)?this.props.payload.fuel_type:[],
  ...
  _open = (menu_type) => {
		if(menu_type == 'car_types'){
			...
			this.setState({ car_types: cSetting,
			day_price: false,
			hour_price: false,
			preferences: false,
			fuel_options: false,
  ...
  funcCarType(id){
	 if(this.state.selected_car_types.length && this.state.selected_car_types.indexOf(id) >=0) {
  ...
  funcFuelType(id){
	  if(this.state.selected_fuel_types.length && this.state.selected_fuel_types.indexOf(id) >=0) {
  ...
  funcPreferenceType(id){
	 if(this.state.selected_preference_types.length && this.state.selected_preference_types.indexOf(id) >=0) {
  ...
  btn_filter = () => {
    if(this.props.callBack){
		var vals = {'car_types':this.state.selected_car_types,'day_price':this.price,'hour_price':this.hourprice,'preferences':this.state.selected_preference_types,'fuel_options':this.state.selected_fuel_types,'seating_capacity':this.seat,'price_type':this.price_type};
		console.log("filter_values selected_preference_types"+JSON.stringify(this.state.selected_preference_types));
  ...
  render = () => {
	...
	for (var i = 0; i < this.props.car_types.length; i++) {
		...
		if(arr_selected_car_types.length && arr_selected_car_types.indexOf(this.props.car_types[i].id) >=0){
			unchecked = checked;
		}
		var id = this.props.car_types[i].id;
		console.log("arr_selected_car_types ="+ id);
		cartypes.push(<View key={i}>
				<TouchableOpacity style={styles.srows} onPress={this.funcCarType.bind(this,this.props.car_types[i].id)}>
	...
	var fueloptions = [];
	if(this.props.vehicle_filters && this.props.vehicle_filters.fuel_type_list){
		console.log("fueltypes aaaa"+JSON.stringify( this.props.vehicle_filters.fuel_type_list));
		var flist = this.props.vehicle_filters.fuel_type_list;
		var i=0;
		Object.entries(flist).forEach(
			([key, value]) => {
				var arr_selected_fuel_types = this.state.selected_fuel_types;
				var unchecked = require('@images/unchecked.png');
				var checked = require('@images/checked.png');
				if(arr_selected_fuel_types.length && arr_selected_fuel_types.indexOf(value) >=0){
					unchecked = checked;
				}
				var id = value;
				console.log("fueltypes ffff ="+ id);
				fueloptions.push(<View key={i}>
						<TouchableOpacity style={styles.srows} onPress={this.funcFuelType.bind(this,id)}>
							<Image style={{width:22,height:22}} source={unchecked} />
							<Text style={[styles.subTitles]}>
								{key}
							</Text>
	...
	var preferences = [];
	var objPreferences = {"ac":Strings.props[this.state.userLang].preferenceairconditioning,"non_ac":Strings.props[this.state.userLang].preferencenonairconditioning,"manual_transmission":Strings.props[this.state.userLang].preferencemanualtransmission,"auto_transmission":Strings.props[this.state.userLang].preferenceautotransmission,"airbag":Strings.props[this.state.userLang].preferenceairbag};

	var i=0;
	Object.entries(objPreferences).forEach(
		([key, value]) => {
			var arr_selected_preference_types = this.state.selected_preference_types;
			var unchecked = require('@images/unchecked.png');
			var checked = require('@images/checked.png');
			if(arr_selected_preference_types.length && arr_selected_preference_types.indexOf(key) >=0){
				unchecked = checked;
			}
			var id = value;
			preferences.push(<View key={i}>
					<TouchableOpacity style={styles.srows} onPress={this.funcPreferenceType.bind(this,key)}>
						<Image style={{width:22,height:22}} source={unchecked} />
						<Text style={[styles.subTitles]}>
							{value}
						</Text>
      ...
      <View style={[styles.menuContainer]}>
        <View style={[styles.menu]}>
			<Animated.View style={{backgroundColor:AppColors.brand.navbar}}>
				{/*car_types*/}
					<TouchableOpacity style={{height:40}} onPress={this._open.bind(this,'car_types')}>
					  <View style={styles.Rows}>
						<View style={styles.RowHeaderView}>
							<View style={{flex:3,flexDirection:'row'}}>
								<Icon name={'md-car'} size={20} color={AppColors.brand.primary} />
								<Text style={[styles.MainTitles]}>
									{Strings.props[this.state.userLang].cartypes}
								</Text>
							</View>
							<View style={{flex:1,justifyContent:'center',alignItems:'flex-end',marginRight:5}}>
								<Icon name={this.state.car_types ? 'md-arrow-dropdown': 'md-arrow-dropright'} size={20} color={'#fff'} />
							</View>
						</View>
					  </View>
					</TouchableOpacity>
					{this.state.car_types ? (
						<View style={{ flexDirection: 'column'}}>
							...
							:<View style={styles.srows}><Text style={[styles.subTitles]}>{Strings.props[this.state.userLang].nocartypes}</Text></View>}
						</View>
					) : (null)}
				{/*car_types*/}
				{/*hour_price*/}
					<TouchableOpacity style={{height:40}} onPress={this._open.bind(this,'hour_price')}>
					  <View style={styles.Rows}>
						<View style={styles.RowHeaderView}>
							<View style={{flex:3,flexDirection:'row'}}>
								<Icon name={'md-pricetag'} size={20} color={AppColors.brand.primary} />
								<Text style={[styles.MainTitles]}>
									{Strings.props[this.state.userLang].hourprice}
								</Text>
							</View>
							<View style={{flex:1,justifyContent:'center',alignItems:'flex-end',marginRight:5}}>
								<Icon name={this.state.hour_price ? 'md-arrow-dropdown': 'md-arrow-dropright'} size={20} color={'#fff'} />
							</View>
						</View>
					  </View>
					</TouchableOpacity>
					{this.state.hour_price ? (
						<View style={{ flexDirection: 'column'}}>
							...
							<View style={styles.srows}>
								 <MultiSlider
									 containerStyle={{
										height:40,justifyContent:'center',alignItems:'flex-start',width:AppSizes.screen.width-40,paddingLeft:10
									  }}
									  touchDimensions={{
										height: 40,
										width: 40,
								...
								<View style = {{ flexDirection: 'row', justifyContent: 'space-between',}}>
										<View style = {{ alignItems: 'flex-start',}}>
											<Text>
												{Strings.props[this.state.userLang].minvalue}
												{this.state.hour_price_min}
											</Text>
										</View>
										<View style = {{ alignItems: 'flex-end',}}>
											<Text>
												{Strings.props[this.state.userLang].maxvalue}
				...
				{/*day_price*/}
					...
					<TouchableOpacity style={{height:40}} onPress={this._open.bind(this,'day_price')}>
					  <View style={styles.Rows}>
						<View style={styles.RowHeaderView}>
							<View style={{flex:3,flexDirection:'row'}}>
								<Icon name={'md-pricetag'} size={20} color={AppColors.brand.primary} />
								<Text style={[styles.MainTitles]}>
									{Strings.props[this.state.userLang].dayprice}
								</Text>
							</View>
							<View style={{flex:1,justifyContent:'center',alignItems:'flex-end',marginRight:5}}>
								<Icon name={this.state.day_price ? 'md-arrow-dropdown': 'md-arrow-dropright'} size={20} color={'#fff'} />
							</View>
						</View>
					  </View>
					</TouchableOpacity>
				...
				{/*day_price*/}
				{/*preferences*/}
					<TouchableOpacity style={{height:40}} onPress={this._open.bind(this,'preferences')}>
					  <View style={styles.Rows}>
						<View style={styles.RowHeaderView}>
							<View style={{flex:3,flexDirection:'row'}}>
								<Icon name={'md-switch'} size={20} color={AppColors.brand.primary} />
								<Text style={[styles.MainTitles]}>
									{Strings.props[this.state.userLang].preferences}
								</Text>
							</View>
							<View style={{flex:1,justifyContent:'center',alignItems:'flex-end',marginRight:5}}>
								<Icon name={this.state.preferences ? 'md-arrow-dropdown': 'md-arrow-dropright'} size={20} color={'#fff'} />
							</View>
						</View>
					  </View>
					</TouchableOpacity>
				...
				{/*preferences Curves*/}
				{/*Tools*/}
					<TouchableOpacity style={{height:40}} onPress={this._open.bind(this,'fuel_options')}>
					  <View style={styles.Rows}>
						<View style={styles.RowHeaderView}>
							<View style={{flex:3,flexDirection:'row'}}>
								<Icon name={'md-flame'} size={20} color={AppColors.brand.primary} />
								<Text style={[styles.MainTitles]}>
									{Strings.props[this.state.userLang].fueloptions}
								</Text>
							</View>
							<View style={{flex:1,justifyContent:'center',alignItems:'flex-end',marginRight:5}}>
								<Icon name={this.state.fuel_options ? 'md-arrow-dropdown': 'md-arrow-dropright'} size={20} color={'#fff'} />
							</View>
						</View>
					  </View>
					</TouchableOpacity>
					{this.state.fuel_options ? (
						<View style={{ flexDirection: 'column'}}>
							{(this.props.vehicle_filters && this.props.vehicle_filters.fuel_type_list) ? 
								<View>
								{fueloptions}
								</View>
							:<View style={styles.srows}><Text style={[styles.subTitles]}>{Strings.props[this.state.userLang].nofueltypes}</Text></View>}
						</View>
					) : (null)}
				{/*Tools*/}
				{/*seating_capacity*/}
					<TouchableOpacity style={{height:40}} onPress={this._open.bind(this,'seating_capacity')}>
					  <View style={styles.Rows}>
						<View style={styles.RowHeaderView}>
							<View style={{flex:3,flexDirection:'row'}}>
								<FA name={'eraser'} size={20} color={AppColors.brand.primary} />
								<Text style={[styles.MainTitles]}>
									{Strings.props[this.state.userLang].seatingcapacity}
								</Text>
							</View>
							<View style={{flex:1,justifyContent:'center',alignItems:'flex-end',marginRight:5}}>
								<Icon name={this.state.seating_capacity ? 'md-arrow-dropdown': 'md-arrow-dropright'} size={20} color={'#fff'} />
							</View>
						</View>
					  </View>
					</TouchableOpacity>
				...
				<View style={{justifyContent:'center',alignItems:'center'}}>
					<Button
						title={Strings.props[this.state.userLang].searchfilter}
